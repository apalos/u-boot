...
static int tpm_tis_read_bytes(u32 addr, u16 len, u8 *result)
{
	while (len--)
		*result++ = ioread8(iobase + addr);
	return 0;
}

static int tpm_tis_write_bytes(u32 addr, u16 len, const u8 *value)
{

	while (len--)
		iowrite8(*value++, iobase + addr);
	return 0;
}

static __maybe_unused int tpm_tis_read16(u32 addr, u16 *result)
{
	*result = ioread16(iobase + addr);
	return 0;
}

static int tpm_tis_read32(u32 addr, u32 *result)
{
	*result = ioread32(iobase + addr);
	return 0;
}

static int tpm_tis_write32(u32 addr, u32 value)
{
	iowrite32(value, iobase + addr);
	return 0;
}
...
static bool tpm_tis_check_locality(struct udevice *udev, int loc)
{
	...
	tpm_tis_read_bytes(TPM_ACCESS(loc), 1, &locality);
...
static int tpm_tis_request_locality(struct udevice *udev, int loc)
{
	...
	tpm_tis_write_bytes(TPM_ACCESS(loc), 1, &buf);
	start = get_timer(0);
	stop = chip->timeout_a;
	do {
		if (tpm_tis_check_locality(udev, loc))
...
static int tpm_tis_status(struct udevice *udev, u8 *status)
{
	...
	tpm_tis_read_bytes(TPM_STS(chip->locality), 1, status);
...
static int tpm_tis_release_locality(struct udevice *udev, int loc)
{
	...
	ret = tpm_tis_write_bytes(TPM_ACCESS(loc), 1, &buf);
	chip->locality = -1;
...
static int tpm_tis_get_burstcount(struct udevice *udev, size_t *burstcount)
{
	...
	stop = chip->timeout_a;
	do {
		tpm_tis_read32(TPM_STS(chip->locality), &burst);
...
static int tpm_tis_ready(struct udevice *dev)
{
	...
	return tpm_tis_write_bytes(TPM_STS(chip->locality), 1, &data);
}
...
static int tpm_tis_send(struct udevice *udev, const u8 *buf, size_t len)
{
	...
	while (len > 0) {
		...
		ret = tpm_tis_write_bytes(TPM_DATA_FIFO(chip->locality),
					  wr_size, buf + sent);
		if (ret < 0)
			goto release_locality;
	...
	/*
	 * Make a final check everything is fine and the TPM expects no
	 * more data
	 */
	ret = tpm_tis_wait_for_stat(udev, TPM_STS_VALID,
                chip->timeout_c, &status);
	...
	ret = tpm_tis_write_bytes(TPM_STS(chip->locality), 1, &data);
	if (ret)
		goto release_locality;
...
static int tpm_tis_recv_data(struct udevice *udev, u8 *buf, size_t count)
{
	...
	while (size < count &&
	       tpm_tis_wait_for_stat(udev,
					 TPM_STS_DATA_AVAIL | TPM_STS_VALID,
					 chip->timeout_c, &status) == 0) {
		tpm_tis_get_burstcount(udev, &burstcnt);
		if (burstcnt < 0)
			return burstcnt;

		len = min_t(int, burstcnt, count - size);
		ret = tpm_tis_read_bytes(TPM_DATA_FIFO(chip->locality), len,
				         buf + size);
		if (ret < 0)
			return ret;
...
static int tpm_tis_probe(struct udevice *udev)
{
	...
	do {
		...
		ret = tpm_tis_read_bytes(TPM_ACCESS(0), 1, &status);
		if (ret)
			break;

		printf("INIT STAT %x\n", status);
		if (status & TPM_ACCESS_VALID) {
			printf("AAAA TPM_ACCESS_VALID\n");
			break;
		}
	} while (get_timer(start) < stop);
	...
	/* Disable interrupts */
	tpm_tis_read32(TPM_INT_ENABLE(chip->locality), &tmp);
	tmp |= TPM_INTF_CMD_READY_INT | TPM_INTF_LOCALITY_CHANGE_INT |
               TPM_INTF_DATA_AVAIL_INT | TPM_INTF_STS_VALID_INT;
	tmp &= ~TPM_GLOBAL_INT_ENABLE;
	tpm_tis_write32(TPM_INT_ENABLE(chip->locality), tmp);

	priv->version = TPM_V2;
	tpm_tis_read_bytes(TPM_RID(chip->locality), 1, &chip->rid);
	tpm_tis_read32(TPM_DID_VID(chip->locality), &chip->vend_dev);
